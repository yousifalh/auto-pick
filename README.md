...
4. How every number was arrived at: [`docs/receipts/`](docs/receipts/) — tool-generated output, regenerated by the command recorded beside each claim, never hand-edited
...
**`docs/superpowers/` was untracked on 2026-08-17 and is not in this repository.** It held the
working record — one spec per investigation, each naming its own baseline commit and test count,
plus nineteen adversarial reviews of this work briefed to invalidate rather than confirm. Those
documents are cited throughout this README, the FDR, and a number of source comments that explain
why a given line is the way it is; roughly 144 such citations now point at files a reader cannot
open. They are left standing rather than stripped, because each one still records *what was done and
when*, and rewriting them would erase that. This paragraph is the answer to all of them.

What survives publicly is the part that was never prose: `docs/receipts/` holds the tool-generated
output every quoted figure comes from, regenerated by the command recorded beside the claim, and
`docs/CORRECTIONS.md` records the figures that moved and why. Where a claim below cites a spec you
cannot read, the receipt named alongside it is the thing to check.
