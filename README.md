...
On a 30-cartridge corpus drawn from 60 held-out synthetic frames, the shipping pipeline — trained detector, trained segmenter, per-frame calibration, packer — puts at least one cell into **12 of 30 cartridges and places 25 cells**. Feeding the same code **ground-truth masks and ground-truth boxes** at each frame's true scale, with the wall inset relaxed to zero so the ceiling is generous rather than a fair fight — an oracle with perception removed from the question — reaches **12 of 30 and 27 cells**. The system is within two cells of that ceiling on this corpus.

`docs/superpowers/specs/2026-08-11-placement-safety.md` §4 (shipping), `2026-08-11-placement-feasibility.md` §5 (oracle), FDR §3. Three caveats the comparison needs. The oracle row was measured on the same 30 instances at commit `9bfc25f`, before the whole-cell occupancy change; on predicted masks that change moved no cell counts, but it was never re-run on ground truth. The two sets of 12 are not the same 12 cartridges — ten overlap. And the oracle is generous on purpose: the same ground truth through the extractor's own 4.25 mm wall inset gives 24 cells in 10 cartridges, *below* what ships, because the shipping segmenter over-reports free floor and some of the 25 exist only because of that.
...
Fed **ground-truth** label maps at each frame's true scale — perfect segmentation, perfect boxes, perfect calibration — this SKU places zero cells in **10 of 10** instances through the shipping extractor. Relaxing the extractor's 4.25 mm wall inset to zero recovers two of the ten, and both are the coin landing on its edge: 64.7 mm and 65.1 mm of free strip against the 65.0 mm the cell needs, surviving only because the occupancy grid quantises in the optimistic direction. Tolerance is not the lever: 18.5 → 18.3 mm recovers 0 instances at any calibration, and on predicted masks the wall inset recovers 0 all the way down to 0.0 mm.
