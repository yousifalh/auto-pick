...
* **`SegmentationPlacementAreaExtractor`** — the path for real imagery. It consumes a trained segmenter's per-pixel label map (`Snapshot.cartridge_masks`, populated by `recog.inference.attach_cartridge_masks`) and arbitrates two *independent* placement estimates via `plan/arbitration.py` — the network's own `bay` channel, and an estimate derived from the other channels it doesn't use. Their agreement (IoU) is a confidence signal, gated at `tau`, whose default is **0.85** (`plan/placement_area.py`'s `SegmentationPlacementAreaExtractor.__init__`) — not the 0.5715 `recog/calibrate_tau.py` calibrated into `configs/planning.yaml`'s `arbitration.tau`, which nothing currently reads into the extractor. Left that way deliberately, not by oversight: FDR §13.2.1 shows the calibration is uninformative (not one of the 35 accepted validation cartridges ever admitted a cell, so the 5% safety budget never actually bound) and 0.85 is the more conservative of the two values.

The split exists because of a hard latency budget, not preference: FDR O3 caps planning's queue rebuild at 8 ms per cartridge, and a single segmenter forward pass alone costs roughly that much on its own. Segmentation therefore runs once per frame, batched, in Recognition (`recog/bay_segmenter.py`) — measured on an RTX 3060 at 40.9 ms for 8 cartridges batched, against 157.0 ms for the same 8 run in a loop (`docs/receipts/seg_eval.txt`), inside vs. well outside the separate 50 ms end-to-end budget respectively (this margin shrank sharply from an earlier 16.7 ms / 60.0 ms measurement on the same hardware — see FDR §13.2.1 for the measurement conditions, including plausible unrelated GPU contention that was not isolated as the sole cause). Planning only ever does mask arithmetic on the already-computed masks (`plan/arbitration.py`, measured ~2 ms per cartridge). `tests/test_planner.py::test_segmentation_extract_arithmetic_stays_under_the_o3_budget` pins that arithmetic-only cost (`extract()` alone — arbitration + rasterisation, not the FFDH pass that follows it) against the 8 ms budget.
