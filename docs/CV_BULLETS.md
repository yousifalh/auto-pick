...
- Built an **oracle evaluation** (ground-truth masks and boxes through the shipping code) showing the shipping system placed **25 cells against the oracle's 27** on a 30-cartridge corpus, and used it to stop further model work on evidence rather than intuition.
...
- Showed one cartridge SKU **unplannable at any perception accuracy** through the shipping planner: its bay is exactly the nominal cell length (65.0 vs 65.0 mm) and an axis-aligned packer loses **0.32 mm per degree** against ±2° jig jitter, so it takes **zero cells in 10 of 10** ground-truth instances — a specification finding, not an accuracy one.
...
- **Know the oracle's three caveats before you are asked.** It was measured one commit earlier, before the whole-cell occupancy change, and never re-run on ground truth. The two sets of 12 share only ten cartridges. And it runs with the wall inset at zero — give it the shipping extractor's own 4.25 mm and it falls to 24 cells in 10 cartridges, below what ships, because the segmenter over-reports free floor. Say "within two cells"; never a percentage, and never "25 of 27" as if the 25 were a subset.
