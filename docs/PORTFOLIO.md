...
At some point the question stops being "can this be better" and becomes "how much better is available". So I built an oracle: the shipping extractor and the shipping packer, fed ground-truth masks and boxes at each frame's true scale, with the wall inset relaxed to zero so it is a generous ceiling rather than a fair fight. It places 27 cells across 12 of 30 cartridges. The shipping system places 25 across 12 — the same count, and ten of the same cartridges. Perfect perception was worth two cells net, so I stopped doing perception work.

Three things keep that from being a stronger claim than it is. The oracle was measured one commit before the whole-cell occupancy change and never re-run on ground truth; on predicted masks that change moved no cell counts, which is evidence but not the same evidence. Give the oracle the extractor's own 4.25 mm wall inset and it drops to 24 cells in 10 cartridges — *below* what ships, because the shipping segmenter over-reports free floor and some of the 25 exist only because of that optimism. And 30 instances is 30 instances.
