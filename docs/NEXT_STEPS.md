...
| D | Integration and arbitration | Planning **2.0 ms/cartridge** vs an 8 ms budget; segmentation 16.2 ms for 8 crops vs 50 ms (was 16.7 ms; an intermediate 40.9 ms reading was GPU-contention noise, since superseded by clean re-measurements — the table is wall-clock and is re-taken every time the receipt is regenerated, and spans 16.2–21.2 / 57.1–88.0 ms across six clean runs, most recently 16.2 / 58.6 ms; see the tray-interior retrain note below) |
