...
**Date:** 14 August 2026, revised 15 August 2026 · repository at `83a1383`,
revisions measured on branch `fix/audit-2026-08-15`
...
instances, and removing the 4.25 mm wall inset entirely recovers **0**. Across
the whole 502-scene corpus the SKU places zero in **47 of 47** instances — no
counter-example anywhere, at any inset or nominal in the sweep. The other
three SKUs carry +1.75, +70.2 and +75.75 mm of margin and are fine; the 13000
is spent by 6.9° of rotation. Receipt: `docs/receipts/placement_feasibility.txt`
§2 and §3, regenerated by `python scripts/placement_feasibility.py`, whose
`--check` reports *no drift: 5 section(s) checked, 0 skipped*.

(**Corrected 2026-08-15.** This paragraph said the zero inset recovers **1** —
a single bay measuring 65.10 mm against a 65.0 mm cell, turned 0.297°,
`scene_00049/item5` — and that the whole-corpus figure was **43 of 47** with
no instance turned more than **0.28°** taking a cell. Those were correct
against the receipt as it stood on 2026-08-14; `cc3fce1` had just changed this
sentence *from* "recovers 0" *to* "recovers 1" for exactly that reason. A
geometry defect fixed on 2026-08-15 has taken the instance away again:
`Planner._pack_cartridge` sized its packing strip from the placement region's
width in millimetres while checking placements against an occupancy grid
rasterised at 1.5 mm, so the strip stood up to one grid cell wider per axis
than the floor it was tested against and a cell could be seated up to 1.5 mm
past the measured placeable edge. Sizing the strip from the grid itself
removes the last such fit in this SKU. The rotation band goes with it: with no
instance taking a cell at any rotation, there is no band to quote. §4 of the
regenerated receipt now marks this figure **[REPRODUCED]** against the wording
above.)
...
segment in **16.2 ms** batched against **58.6 ms** one at a time, so batching
...
**Reproducibility.** The suite is **1,396 passed / 1 skipped** of 1,397
collected at HEAD with torch installed. Branch coverage is **93 %** over
`main.py`'s 19-module transitive import closure (1,845 statements) and **67 %**
over all 49 modules the coverage config resolves to (7,208 statements), the
second figure below the 70 % threshold and on the record because Blender-only
renderers cannot be unit-tested outside Blender (`pytest-cov.txt`). **Those two
coverage figures are pinned to commit `885a044` and a suite of 1,210 tests, and
have not been re-measured since** — `pytest-cov.txt` states its own scope as a
clean worktree of that commit, and re-running it needs a full-suite
`pytest --cov` on a clean checkout rather than a shared working tree. The
1,210 / 2 pair this paragraph used to lead with is that receipt's, not HEAD's,
and it is now 187 tests behind; the coverage percentages are quoted as
historical for the same reason. Training is seeded: the same seed twice gives bit-identical
...
| Boundary displacement 1.226 mm, 3.14× margin; 16.2 vs 58.6 ms at 8 cartridges; per-frame GSD 0.4903–1.0739 mm/px | `docs/receipts/seg_eval.txt` |
...
| 1,210 passed / 2 skipped; 93 % and 67 % branch coverage — **as measured at commit `885a044`; the suite is 1,396 / 1 of 1,397 at HEAD and the coverage figures are unrefreshed** | `docs/receipts/pytest-cov.txt` |
...
| 10 of 10 zero-placement instances on ground-truth masks (47 of 47 corpus-wide); GT oracle 9 of 29 open cartridges / 23 cells at every wall inset | `docs/receipts/placement_feasibility.txt` |
