...
Segmentation itself runs at **18.1 ms for 8 crops batched**, inside
...
checkpoint, same warm-up, give 18.1 ms batched against **61.8 ms if
they are segmented one at a time (3.4×)**, which breaches the
...
receipt; this is the one to cite going forward. A deployment that
quietly loses batching — a frame with more cartridges than the batch
was sized for, or a fallback to fp32 — breaks the budget rather than
degrading gracefully.

**Δcells is the packing-level number, and it needed re-measuring.**
Mask IoU is an intermediate; what the packer cares about is cells.
The figure previously reported here (+0.148 cells, 0 of 54 negative)
was produced by a packer-count helper that quantised the forbidden
mask at an effective ~1.25 mm/cell stride while telling the packer
...
review). Re-measured using `plan.placement_area._rasterise_mask` —
production's own rasteriser, not a re-derived stride — running the
fixed packer of §6.3.1 on the ground-truth mask and on the predicted
mask gives a mean difference of **+0.037 cells** over the same 54
validation crops: **51 of 54 exact**, 1 losing a cell to conservatism,
and — the figure that matters for safety — **2 of 54 in the negative
direction** (-1 and -3 cells). Positive means cells the ground truth
would have placed and the prediction gave up; negative means cells
packed where the truth forbids them, the direction that puts a cell
on a PCB. **The earlier "0 of 54 negative" claim does not survive
this correction and should not be repeated.** Both negative crops
were inspected individually: in each, the ground-truth safe region
covers MORE pixels than the prediction's, but is shaped too thin and
elongated to admit a full cell footprint anywhere, while the
prediction's smaller, more compact region does — the same
morphological-vs-areal distinction `plan.arbitration.admits_a_cell`
exists to catch (Task 2's blob-vs-rim demonstration), now surfacing
in the packing-level number rather than only the tau calibration one.
Two caveats carry over unchanged: this is measured on the segmenter's
own synthetic validation split, because Δcells needs a ground-truth
label map and `recog/realtest/` has none; and it is gated on the
§6.3.1 packer fix, since on the unfixed shelf-cursor logic the figure
was dominated by shelf abandonment rather than by mask error.
...
and Δcells (§13.2.1) — corrected during final review to a +0.037
mean with 2 of 54 crops in the damage direction, not the 0 of 54
originally reported (a packer-count stride bug, since fixed). Bounded
by sample size: τ, on 19 cartridges none of which exercised the
